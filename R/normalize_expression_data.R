...
#' negative values. The expression data must contain gene identifier as first
#' column, followed by the data columns for each individual sample. This
#' function is agnostic to the identifier column name (hugo symbol og ENSG ids)
#' and expects the data to contain more than one unique sample in order to
#' properly calculate the z-score. This implementation uses the whole population
...
#'
#' @param expression_df Input data frame with raw gene expression counts.
